...
public class SeriesSlicePersistHandlerTest {
    ...
    public void testPersistPrimitive() throws Exception {
        SeriesSlicePersistHandler<Double> handler =
                new SeriesSlicePersistHandler<>(new DoubleSerializer(),
        ...
        TimeSeriesConfiguration configuration =
                new TimeSeriesConfiguration(10, 1, 0);

        Key key = handler.persist(configuration, TS_ID, slice);

        assertNotNull(key);
        assertNotNull(DatastoreServiceFactory.getDatastoreService().get(key));

        List<SeriesSlice<Double>> list =
                handler.query(configuration, TS_ID, 0, 1);
        ...
        assertEquals(slice.getMaxResolution(),
                retrievedSlice.getMaxResolution());
    ...
    public void testPersistObject() throws Exception {
        SeriesSlicePersistHandler<TSTestValue> handler =
                new SeriesSlicePersistHandler<>(
                        new SmallSerializableObjectSerializer(
                                TSTestValue.class),
                        DatastoreServiceFactory.getDatastoreService());
        ...
        List<SeriesSlice<TSTestValue>> list =
                handler.query(configuration, TS_ID, 0, 1);
